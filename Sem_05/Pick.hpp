...
template <typename T>
int partitionPick(T arr[], int len, int splitter, bool smallerFirst)
{
...
T PICK(T A[], int n, int k)
{
	...
	while (true)
	{
		...
		if (k <= L1)
			n = partitionPick(A, n, M, true); // Put the small numbers at the beginning:
		else
		{
			n = partitionPick(A, n, M, false); // Put the big numbers at the beginning:
			k -= L2;
		}
